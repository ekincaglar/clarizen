...
{
    public class crossOrgEntityQuery : ISupportBulk
    {
        public Result.crossOrgEntityQuery Data { get; set; }
        ...
        public async Task Call(Queries.crossOrgEntityQuery request, CallSettings callSettings)
        {
            // Set the URL
            string url = (callSettings.isBulk ? string.Empty : callSettings.serverLocation) + "/data/crossOrgEntityQuery";
            ...
            else if (response.Status == System.Net.HttpStatusCode.OK)
            {
                try
                {
                    this.Data = JsonConvert.DeserializeObject<Result.crossOrgEntityQuery>(response.Content, new JsonSerializerSettings()
                    {
