...
{
    public class expenseQuery : ISupportBulk
    {
        public Result.expenseQuery Data { get; set; }
        ...
        public async Task Call(Queries.expenseQuery request, CallSettings callSettings)
        {
            // Set the URL
            string url = (callSettings.isBulk ? string.Empty : callSettings.serverLocation) + "/data/expenseQuery";

            if (callSettings.isBulk)
            {
                this.BulkRequest = new request(url, requestMethod.Post, request, typeof(Result.expenseQuery));
                return;
            }
            ...
            if (response.Status == System.Net.HttpStatusCode.OK)
            {
                try
                {
                    this.Data = JsonConvert.DeserializeObject<Result.expenseQuery>(response.Content);
