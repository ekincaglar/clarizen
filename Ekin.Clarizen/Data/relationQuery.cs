...
{
    public class relationQuery : ISupportBulk
    {
        public Result.relationQuery Data { get; set; }
        ...
        public async Task Call(Queries.relationQuery request, CallSettings callSettings)
        {
            // Set the URL
            string url = (callSettings.isBulk ? string.Empty : callSettings.serverLocation) + "/data/relationQuery";

            if (callSettings.isBulk)
            {
                this.BulkRequest = new request(url, requestMethod.Post, request, typeof(Result.relationQuery));
                return;
            }
            ...
            if (response.Status == System.Net.HttpStatusCode.OK)
            {
                try
                {
                    this.Data = JsonConvert.DeserializeObject<Result.relationQuery>(response.Content);
